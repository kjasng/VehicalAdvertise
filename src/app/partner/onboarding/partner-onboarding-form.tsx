...
export function PartnerOnboardingForm({ status, rejectReason, existingData }: Props) {
  ...
  if (status === 'pending') {
    return (
      <div className="flex flex-col items-center gap-4 py-16 text-center">
        <Clock className="size-12 text-[#999]" aria-hidden="true" />
        <h2 className="font-heading text-[24px] text-[#1a1a1a] uppercase">Đang chờ xét duyệt</h2>
        <p className="max-w-[400px] text-[14px] text-[#666666]">
          Hồ sơ công ty của bạn đang được xem xét. Chúng tôi sẽ gửi email thông báo trong 24 giờ.
        </p>
      </div>
    )
  }
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      ...
      else toast.success('Hồ sơ đã được gửi. Chúng tôi sẽ phản hồi trong 24 giờ.')
    })
  }
